...
export function NewNoteCard({onNoteCreated}: NewNoteCardProps) {
  ...
  function handleStartRecording(){
    ...
    if(!isSpeechRecognitionAPIAvailable){
      alert('infelizmente seu navegador nao suporta a api de navegacao')
      return
    }
  ...
  return (
    <Dialog.Root>
      <Dialog.Trigger className="bg-slate-700 rounded-md p-5 text-left gap-3 flex flex-col hover:ring-2    hover:ring-slate-600 overflow-hidden focus-visible:ring-2 focus-visible:ring-lime-400 outline-none">
        <span className="text-sm text-slate-200">Adicionar Nota</span>
        <p className="text-sm leading-6 text-slate-400">
          Grave uma nota em audio
        </p>
      </Dialog.Trigger>
      <Dialog.Portal>
        <Dialog.Overlay className="inset-0 fixed bg-black/50" />
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2  md:-translate-y-1/2 md:-translate-x-1/2 md:max-w-[640px] md:w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col   outline-none">
